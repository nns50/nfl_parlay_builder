...
COMMON="The session_start.sh digest is already in your context (injected by hook) — selftest, store sync, quota/ODDS_MODE, unsettled proposals, weekcheck, availability, pulse. If the digest is NOT in context (hook did not fire), run bash tools/session_start.sh first. Doctrine: CLAUDE.md. Git — BEFORE working: git fetch origin; if any origin/claude/admiring-johnson-* tip is AHEAD of your clone's base, merge those branches in first (ledgers/builds are append-only, the merge is clean) — this keeps the ledger accumulating when a prior run's branch has not been folded into main yet. Then commit + push. Trigger-fired sessions push to their per-run outcome branch (claude/admiring-johnson-*) — expected and correct (proven 2026-08-09): state the branch + SHA in the notification and flag that it needs folding into main (the dashboard deploys from main). Interactive sessions push to the designated feature branch (NO auto-merge — not granted for this repo). If a push is rejected as unauthorized, load mcp__Claude_Code_Remote__add_repo via ToolSearch (owner nns50, repo nfl_parlay_builder, access push) and retry; if still blocked, include the commit SHA in the notification so the work is traceable."
